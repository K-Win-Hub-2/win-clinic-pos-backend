...
exports.getAppointByDoctorId = async (req,res)=>{
     let findDoctorOrTherapist,
         commissionResult 
     let { name, code, isDoctor, isNurse, startDate, endDate } = req.body;
     let query = { isDeleted:false, name:name, code: code};
     if(isDoctor){
        findDoctorOrTherapist = await Doctor.findOne(query).select("-code"); 
        commissionResult = await Comission.find({relatedDoctor: findDoctorOrTherapist._id})
                                 .populate("relatedTherapist relatedAppointment relatedDoctor relatedTreatmentVoucher")
                                 .populate({
        ...
        commissionResult = await Comission.find({relatedNurse: findDoctorOrTherapist._id})
                                 .populate("relatedTherapist relatedAppointment relatedDoctor relatedTreatmentVoucher")
                                 .populate({
                                    path:"relatedAppointment",
                                    populate:[{
        ...
        commissionResult = await Comission.find({relatedTherapist: findDoctorOrTherapist._id})
                                 .populate("relatedTherapist relatedDoctor relatedTreatmentVoucher") 
                                 .populate({
                                    path:"relatedAppointment",
                                    populate:[{
                                        path: "relatedPatient"
